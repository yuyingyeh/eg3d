...
def init_dataset_kwargs(data):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.SvbrdfLowFolderDataset',
                                            path=data, use_labels=False, max_size=None, xflip=False)
...
def main(**kwargs):
    ...
    c.G_kwargs.class_name = 'training.monoplane.MonoPlaneNoSRGenerator'
    ...
    # Description string.
    desc = f'{opts.cfg:s}-{dataset_name:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma:g}-glr{c.G_opt_kwargs.lr:g}-dlr{c.D_opt_kwargs.lr:g}'
